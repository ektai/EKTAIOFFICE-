...
cp -r ../../Files/EKTAIOFFICE/. "$RPM_BUILD_ROOT/var/www/%{package_sysname}/"
...
cp ../../Files/nginx/EKTAIOFFICE.conf "$RPM_BUILD_ROOT%{nginx_conf_d}/"
...
-depth -type f \
-exec sed -i 's/EKTAIOFFICE/%{package_sysname}/g' {} \; \
-exec sed -i 's/EKTAIOFFICE/%{package_sysname}/g' {} \; \
-exec sed -i 's/{{NGINX_USER}}/%{nginx_user}/g' {} \; \
-exec sed -i 's!{{NGINX_CONF_DIR}}!%{nginx_conf_d}!g' {} \; \
-exec rename -f -v 's/EKTAIOFFICE([^\/]*)$/%{package_sysname}$1/g' {} \;

find "$RPM_BUILD_ROOT/var/www/%{package_sysname}/Sql" -depth -type f -exec rename -f -v 's/EKTAIOFFICE([^\/]*)$/%{package_sysname}$1/g' {} \;
...
for FILE in `find "$RPM_BUILD_ROOT/var/www/%{package_sysname}/"`; do
	...
	if [ -d "$FILE" ]; then
		echo "%%attr(-, %{package_sysname}, %{package_sysname}) %%dir \"$RELFILE\"" >>EKTAIOFFICE.list
	else
		case "$FILE" in
			*/WebStudio/web.connections.config )
				echo "%%attr(-, %{package_sysname}, %{package_sysname}) %%config(noreplace) \"$RELFILE\"" >>EKTAIOFFICE.list
			;;
			...
			*/WebStudio/[Ww]eb*.config | */ApiSystem/[Ww]eb*.config | */TeamLabSvc.exe.config | */ASC.Mail.StorageCleaner.exe.config | */ASC.Mail.Aggregator.CollectionService.exe.config | */ASC.Mail.Watchdog.Service.exe.config | */ASC.Mail.EmlDownloader.exe.config )
				echo "%%attr(-, %{package_sysname}, %{package_sysname}) %%config \"$RELFILE\"" >>EKTAIOFFICE.list
			;;

			* )
				echo "%%attr(-, %{package_sysname}, %{package_sysname}) \"$RELFILE\"" >>EKTAIOFFICE.list
			;;
		esac
	fi
done
