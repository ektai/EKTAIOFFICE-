...
%global package_header_tag_name EKTAIOFFICE-communityserver
%global package_header_tag_url http://EKTAIOFFICE.com/
%global package_header_tag_vendor Ascensio System SIA
%global package_header_tag_packager Ascensio System SIA <support@EKTAIOFFICE.com>
...
%global package_services monoserve monoserveApiSystem EKTAIOFFICESocketIO EKTAIOFFICEThumb EKTAIOFFICEBackup EKTAIOFFICEFeed EKTAIOFFICEJabber EKTAIOFFICEIndex EKTAIOFFICENotify EKTAIOFFICEMailAggregator EKTAIOFFICEMailWatchdog EKTAIOFFICEMailCleaner EKTAIOFFICEStorageMigrate EKTAIOFFICEUrlShortener EKTAIOFFICERadicale

%global package_sysname EKTAIOFFICE
...
%changelog
* %{package_header_tag_date} %{package_header_tag_vendor} <support@EKTAIOFFICE.com> %{package_header_tag_version}-%{package_header_tag_release}
...
- Added the generation of the unique machinekey when installing the Docker containers using script; the key is stored in the /app/EKTAIOFFICE/CommunityServer/data/.private hidden folder;
...
- Added the possibility to run an external script from the /var/vmail/external.sh or /app/EKTAIOFFICE/MailServer/data/external.sh path;
...
- Added the new Python scripts allowing to create mailboxes, change password and run imapsync batch in /usr/src/iRedMail/tools/scripts folder inside the EKTAIOFFICE-mail-server container;
